"""
Module for creating UI Selection Page.
"""
...
class SelectionPage(page.ViewPage):
    ...
    **kwargs : dict, optional
    ...
    def show(self, *args: Any) -> None:
        ...
        # Align widgets
        selection_text.grid(sticky="n", pady=(0, 10))
    ...
    def display_repo_params(self, *args: Any) -> None:
        ...
        if self.controller.model.requirements.get("search_terms"):
            ...
            search_term_label = ttk.Label(
                master=search_term_frame, text="Search Term(s):"
            )
            ...
            search_term_req = ttk.Label(
                master=search_term_frame, foreground="#FF0000", text="*"
            )
        ...
        if self.controller.model.requirements.get("search_types"):
            search_type_options = (
                self.controller.model.collector_class.search_type_options
            )
            ...
            search_type_label = ttk.Label(
                master=search_type_outer_frame, text="Search Type(s):"
            )
            search_type_req = ttk.Label(
                master=search_type_outer_frame, foreground="#FF0000", text="*"
            )

            self.controller.add_run_parameter(
                param="search_types",
                value={search_type: tk.IntVar() for search_type in search_type_options},
            )
            ...
            for search_type in search_type_options:
                search_type_button = ttk.Checkbutton(
                    ...
                    variable=self.controller.get_run_parameter("search_types")[
                        search_type
                    ],
                )
    ...
    def alert_missing_reqs(self, missing_requirements: list[str]) -> None:
        ...
        try:
            ...
            self.req_var.set(f"Missing Requirements:\n{req_list}")
            self.req_label = ttk.Label(
                master=self.param_frame, foreground="#FF0000", textvariable=self.req_var
            )
    ...
    def _toggle_button_state(
        ...
        if not all(hasattr(var, "get") for var in toggle_vars):
            raise TypeError('All entries of toggle_vars must implement "get" method.')
